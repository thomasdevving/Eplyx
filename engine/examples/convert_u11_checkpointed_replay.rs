//! Convert the retained checkpointed replay evidence into one ordinary schema-2
...
use eplyx_engine::{
    ...
    universal::{
        checkpoint::{
            CheckpointAccount, DerivedAccountEvidenceV1, FailedOverlap,
            FailedTransactionRollbackRule, ObservedCheckpointV1, TransactionClosureProofV1,
        },
        evidence::{
...
fn derived_runtime_account(
    ...
    let evidence = DerivedAccountEvidenceV1 {
        schema_version: 1,
        ...
        content,
        sources: vec![source],
        derivation: "deterministic runtime account reconstruction from retained validator event"
            .into(),
    };
    Ok((evidence.store(store)?, account))
}
