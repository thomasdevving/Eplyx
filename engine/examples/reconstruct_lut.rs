    ...
    for row in rows {
        ...
        match result {
            Ok(proven) => {
                ...
                output.push(json!({"signature":row["result"]["transaction"]["signatures"][0],"proof":proof,"failure":null,"passed_previous_lut_gate":true,"next_old_engine_rejection":next,"production_replay_eligible":false}));
            },
