...
pub fn capture(
    ...
    for (n, base) in prepared.fixtures.iter().enumerate() {
        ...
        let record=ReplayRecord {schema_version:1,id:base.id.clone(),program_id:crate::fixture_program_id().to_string(),genesis_hash:genesis.clone(),
            current_program_sha256:program_hash.clone(),clock,state_source:ReplayStateSource::ControlledSnapshot,
            pre_state_hash:state_hash(&pre)?,original:Some(OriginalExecution{success:transaction.success,fee:transaction.fee,post_state_hash:state_hash(&post)?}),
