...
**Phase P2 made hosted runs durable.** Candidates live in a global, immutable,
content-addressed store (`server/src/artifacts.rs`, `artifacts/programs/<sha256>`):
temp → fsync → verify → `hard_link` (write-once), dedup by hash, and every read
checks existence, length and SHA-256. The artefact and the hash-pinned
`expected-changes.toml` are persisted **before** the run record, and the record
is written last, so an accepted run never names an input the service does not
hold. The worker reads nothing by filename: `candidate_artifact == spec.candidate
== sha256(bytes read)`, and it hands the engine the verified bytes. `runs/<id>/work/`
is a scratch cache only. The run record is the queue entry: `serve` takes an
exclusive lock on the volume (`data/.lock`), sweeps `artifacts/tmp/`, and then
`Registry::recover_runs` requeues queued runs. A `running` run is finalized from a
complete, verified `report.json` if one exists; otherwise its attempt becomes
`interrupted` and the run is requeued, up to `MAX_EXECUTION_ATTEMPTS` (3). A
retry is an attempt of the same run, never a new run. Pre-P2 non-terminal runs
become `execution_error` and are never given invented inputs. **Artefacts are
never deleted**; a future collector must count references from run records.
Artefact visibility and reuse (`GET /v1/projects/{p}/artifacts/{sha}`, and a
...
Two findings matter beyond the adapter. **Kamino semantics are not contained in one instruction**: both actions require `refreshReserve` and `refreshObligation` in the same transaction naming the same accounts, and the adapter requires them rather than tolerating them — the multi-instruction shape the first two adapters never exercised. And **token flow alone cannot establish a borrow**: two candidates can credit a borrower identically and record different debts, which `token_flow_alone_cannot_establish_the_borrow_semantic` proves and which is why `FlowEvidence` has no `is_empty`.

**Production replay-eligibility for the slice is zero, and that is an engine limit rather than an adapter one.** Of 110 sampled mainnet transactions carrying a top-level KLend instruction, 104 (94.5%) resolve address lookup tables; the two eligible deposits are wrapped-SOL flows needing `SyncNative` and `CloseAccount`. Both blockers — lookup-table execution and account-lifecycle replay — are on the not-started list. The layout is instead pinned to real deployed bytes: `docs/examples/mainnet-kamino-accounts.json` holds a matched reserve and obligation that cross-reference each other. `SemanticAction` was **not** extended — a borrow reports `Unknown` with an exact `action_id` rather than being misfiled as `Withdraw`. See `docs/phase-u2-kamino.md`.
