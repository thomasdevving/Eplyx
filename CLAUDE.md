...
- **Whether a change is *intended* is deliberately not classified.** A legitimate upgrade may change behaviour on purpose. Phase 7 quantifies the change; expected-vs-unexpected classification is a later phase.
...
**Bounded, not general.** The CPI path is one protocol (`SPoo1Ku8…`), one instruction (`DepositSol` into a pool with no SOL deposit authority), one level of invocation, into two known programs. Do not describe it as CPI support.

Explicitly **not** started, and not to be begun without being asked: general CPI execution beyond that contract, address lookup table execution, account creation/closure in replay, failed-original replay, multi-instruction sequence search, expected-vs-unexpected change classification, protocols beyond Token-2022 and SPL Stake Pool (lending, vaults, AMMs), GitHub Actions / CI integration, frontend UI, AI analysis, and fiat valuation of protocol assets.
