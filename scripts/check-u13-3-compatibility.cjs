#!/usr/bin/env node
// Read-only frozen product controls; writes only the new U13.3 receipt.
const fs = require('node:fs');
const crypto = require('node:crypto');
const { spawnSync } = require('node:child_process');
const root = 'docs/examples/phase-u13-3-validation';
fs.mkdirSync(root, { recursive: true });
const suffix = process.platform === 'win32' ? '.exe' : '';
const cli = `target/debug/eplyx${suffix}`;
const u10 = `target/debug/examples/qualify_u10_historical_replay${suffix}`;
...
const report = { schema: 'U13_3CompatibilityControlsV1', results,
