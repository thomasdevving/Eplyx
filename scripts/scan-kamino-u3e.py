...
OUTPUT = lut.REPO / 'docs/examples/phase-u3e-validation/security-scan.json'
...
def main():
    ...
    for root in sorted((lut.REPO / 'docs/examples').glob('phase-u3*')):
        for path in sorted(root.rglob('*')):
            ...
            files.append({'path': str(path.relative_to(lut.REPO)), 'sha256': lut.sha(content)})
    doc = lut.REPO / 'docs/phase-u3e-t1-state-and-runtime.md'
    if doc.exists():
        check(doc.read_text())
        files.append({'path': str(doc.relative_to(lut.REPO)), 'sha256': lut.sha(doc.read_bytes())})
    for pattern in ('*u3e*.py', 'historical_transport.py', 'test_historical_transport.py'):
